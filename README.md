...
- **词库对照**：中文名与日文名分行展示（不合并）；标签别名自动归一（如 中出し→中出）；已知演员/标签从本地词库直取，未收录的自动学习沉淀
...
| **元数据骨架文案 + av 词库** | 番号/简介/演员（中文名）/原名（日文名）/标签/类别 六段式；JavBus/JavLibrary/getav 多源补全 + 本地词库对照（演员中日名互查、标签别名归一、类别派生、番号快照降级重放） |
...
| `av_tag` | 标签规范名（aliases、category） | 标签别名归一（中出し→中出）+ 类别派生（巨乳→巨乳系） |
| `av_code` | 番号 → 元数据快照（v2 字段白名单，30 天有效） | 在线源**全失败**时降级重放；部分补齐只合并不覆盖 |

- **种子数据**（`avdict_seed.py`）：18 类别 / 151 常见标签 / 117 别名，运行时以库为准；未收录条目自动回写（学习回路）
...
- **演员两行分离**：中文名（演员行）与日文名（原先行）独立 hashtag 行，兼容解析旧「中文名 (日文名)」合并格式
...
│   ├── avdict.py        # av 词库：演员对照/标签归一/类别派生/番号快照（熔断降级）
│   ├── avdict_seed.py   # 词库种子：18 类别/151 标签/117 别名（运行时以库为准）
