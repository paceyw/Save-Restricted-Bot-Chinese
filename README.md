...
- 成品以相册投递（封面 + 视频，>1.8GB 自动关键帧无损分段）；caption 五段式：番号/简介/演员/标签/类别（缺数据留空占位）
...
| **元数据五段式文案** | 番号/简介/演员双名（中文名 (日文名)）/标签/类别 + 版本 badge；JavBus/JavLibrary/getav 多源补全 |
...
演员/标签/类型**当前无本地词库**，全部字段实时抓取：演员中文名来自 JavBus CN / JavLibrary CN / getav `starsZh` / missav `/cn/` 页（四源天然中文），日文名来自 missav 页面板与 getav `stars`；标签类型中文来自各站页面（天然中文，无需翻译词库）。

**逐步完善方案（建议，未实施）**：新增 mongo 集合 `av_dict` 沉淀 `code_meta`（番号 → 元数据快照，源失败兜底重放）与 `actress_map`（日文名 ↔ 中文名 ↔ 英文名映射，成功配对即沉淀）；查询顺序 本地缓存 → 在线源 → 回写。**是否实施待拍板。**
